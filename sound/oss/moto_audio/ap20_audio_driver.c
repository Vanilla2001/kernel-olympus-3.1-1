	...
	if (file->f_flags & O_TRUNC) {
		...
		if (primary_spkr_setting == CPCAP_AUDIO_OUT_LOUDSPEAKER) {
			...
			cpcap_audio_state.microphone =
						CPCAP_AUDIO_IN_PRIMARY_INTERNAL;
			primary_spkr_setting = CPCAP_AUDIO_OUT_HANDSET;
			mic_setting = CPCAP_AUDIO_IN_PRIMARY_INTERNAL;
		} else {
			cpcap_audio_state.codec_primary_speaker =
							primary_spkr_setting;
		...
		if (file->f_mode & FMODE_READ) {
		AUDIO_LEVEL1_LOG("CODEC in recorder mode called  mic  0x%x \n",mic_setting);
			cpcap_audio_state.microphone = mic_setting;
			cpcap_audio_state.stdac_primary_speaker =32;
			cpcap_audio_state.stdac_mode =2;
			cpcap_audio_state.stdac_mute = 0;
...
MODULE_DESCRIPTION("CPCAP audio driver");
